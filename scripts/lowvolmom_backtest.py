"""Low-Volatility Momentum backtest.

Idea: combine quality (12-month momentum) with stability (low realized
volatility). The hypothesis is that low-vol high-momentum names produce
smoother equity curves and a higher win rate than pure momentum, because the
target gets hit before the stop more often when daily moves are calmer.

Strategy:
  - Each month-end: for each eligible ticker compute
      * 12-month return (skip-the-last-month not applied — straight 12mo
        return is enough; this combo focuses on the vol overlay)
      * 60-day realized volatility = std(daily returns) * sqrt(252)
  - Take top 30% by 12-month return (the momentum filter — quality)
  - Within survivors, pick the BOTTOM 10 by 60-day volatility (the stability
    filter — lowest-vol of the high-momentum names)
  - At next month open: BUY top-10 candidates at $1,000 each
  - Each position has:
      stop:    entry * 0.94  (6% below)
      target:  entry * 1.18  (18% above, 1:3 R:R)
      time:    90 days max
  - Exits: stop / target / time-stop, whichever first (intra-day OHLC check)
  - Open positions are not force-closed on rebalance
  - Max 10 concurrent positions

Universe filters (at signal date):
  - Price between $10 and $200
  - 50-day avg volume > 500k shares
  - >= 270 trading days of history (need 12-month lookback)
"""
...
MAX_POSITIONS = 10
TOP_N = 10  # final basket size (lowest-vol of high-momentum names)

# Selection
MOMENTUM_TOP_PCT = 0.30  # top 30% by 12mo return (momentum filter)
VOL_LOOKBACK_DAYS = 60  # realized vol window
MIN_VOL_DAYS = 50  # require at least this many returns inside the vol window
...
def load_bars() -> dict[str, pd.DataFrame]:
    ...
    for f in files:
        ...
        df["vol_60d"] = df["ret"].rolling(VOL_LOOKBACK_DAYS, min_periods=MIN_VOL_DAYS).std() * np.sqrt(252)
        bars[f.stem] = df
...
def select_lowvol_momentum(
    bars: dict[str, pd.DataFrame], rank_date: pd.Timestamp, top_n: int
) -> list[tuple[str, float, float]]:
    """Pick basket: top-30% by 12mo return, then bottom-N by 60d vol.

    Returns list of (ticker, momentum_12m, vol_60d).
    """
    candidates: list[tuple[str, float, float]] = []  # (ticker, momentum, vol)
    target_lookback = rank_date - pd.DateOffset(months=12)

    for sym, df in bars.items():
        # Locate the closest trading day on or before rank_date
        if rank_date in df.index:
        ...
        # 12mo return (skip-the-last-month not applied for this combo)
        lookback_pos = df.index.get_indexer([target_lookback], method="ffill")[0]
        if lookback_pos < 0 or lookback_pos >= idx_now:
            continue
        p_12mo = float(df.iloc[lookback_pos]["close"])
        p_now = float(df.iloc[idx_now]["close"])
        if p_12mo <= 0 or p_now <= 0:
            continue
        mom_12m = p_now / p_12mo - 1.0

        # 60d realized vol at the ranking date
        vol_60d = float(df.iloc[idx_now]["vol_60d"]) if "vol_60d" in df.columns else float("nan")
        if not np.isfinite(vol_60d) or vol_60d <= 0:
            continue
        ...
        candidates.append((sym, mom_12m, vol_60d))

    if not candidates:
        return []

    # Step 1: keep top 30% by momentum
    candidates.sort(key=lambda x: x[1], reverse=True)
    cutoff = max(top_n, int(len(candidates) * MOMENTUM_TOP_PCT))
    high_mom = candidates[:cutoff]

    # Step 2: within high-momentum survivors, pick the lowest-vol N
    high_mom.sort(key=lambda x: x[2])  # ascending by vol
    return high_mom[:top_n]
...
def run_backtest(
    ...
    print(f"\nBacktest Low-Vol Momentum: {BACKTEST_START.date()} -> {BACKTEST_END.date()}")
    print(
        f"  Top {MOMENTUM_TOP_PCT:.0%} by 12mo return -> bottom {TOP_N} by 60d vol, "
        f"${DOLLAR_PER_TRADE}/trade, stop {STOP_PCT:.0%}, target {TARGET_PCT:.0%}, time {TIME_STOP_DAYS}d\n"
    )
    ...
    pending_buys: list[tuple[str, float, float]] = []  # (sym, mom, vol)
    ...
    for today_date in trading_days:
        ...
        for sym, mom, vol in pending_buys[:]:
            if sym in open_positions:
                pending_buys.remove((sym, mom, vol))
                continue
            if sym not in bars or today_date not in bars[sym].index:
                continue
        ...
        if (
            next_rebalance_idx < len(month_ends)
            and today_date >= month_ends[next_rebalance_idx]
        ):
            top = select_lowvol_momentum(bars, today_date, TOP_N)
            for sym, mom, vol in top:
                if sym not in open_positions and not any(p[0] == sym for p in pending_buys):
                    pending_buys.append((sym, mom, vol))
            ...
            if len(closed_trades) > 0 and next_rebalance_idx % 12 == 0:
                ...
                top_str = [(s, f"mom={m:.0%}", f"vol={v:.0%}") for s, m, v in top[:3]]
                print(
...
def report(trades: list[Trade], equity: pd.DataFrame, spy: pd.DataFrame) -> None:
    print("\n" + "=" * 60)
    print("  RESULTS — Low-Volatility Momentum")
    print("=" * 60)
    if not trades:
        print("No trades.")
        return
    ...
    print(f"Avg 60d vol at entry:     {df['vol_60d'].mean():.1%}")
    ...
    if not spy_aligned.empty and not pd.isna(spy_aligned.iloc[0]):
        ...
        spy_sharpe = (
            spy_daily.mean() / spy_daily.std() * np.sqrt(252)
            if spy_daily.std() > 0
            else 0
        )
        spy_rolling_max = spy_aligned.cummax()
        spy_dd = (spy_aligned - spy_rolling_max) / spy_rolling_max
        print("\nSPY benchmark:")
        print(f"  Total return:           {spy_return:+.1%}")
        print(f"  CAGR:                   {spy_cagr:+.1%}")
        print(f"  Max drawdown:           {spy_dd.min():.1%}")
