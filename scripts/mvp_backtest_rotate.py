...
def run_backtest(bars: dict[str, pd.DataFrame], spy: pd.DataFrame) -> tuple[list[Trade], pd.DataFrame]:
    ...
    for day_idx, today_date in enumerate(trading_days):
        ...
        # Phase 3: BUY best candidates until slot/budget limits hit (with ROTATION)
        ROTATION_RATIO = 1.5  # new signal must be 1.5x weakest open's volume ratio
        for sym, df, idx, base, vol_ratio in candidates:
            # If slots full, try ROTATION: replace weakest open trade if new signal is much stronger
            if len(open_positions) >= MAX_POSITIONS:
                weakest_sym = min(open_positions, key=lambda s: open_positions[s].breakout_volume_ratio)
                weakest_trade = open_positions[weakest_sym]
                if vol_ratio > ROTATION_RATIO * weakest_trade.breakout_volume_ratio:
                    # Rotate: force-exit weakest at today's close
                    if weakest_sym in bars and today_date in bars[weakest_sym].index:
                        exit_price = float(bars[weakest_sym].loc[today_date, "close"]) * (1 - SLIPPAGE_EXIT)
                        weakest_trade.exit_date = today_date
                        weakest_trade.exit_price = exit_price
                        weakest_trade.exit_reason = "rotation"
                        risk_per_share_w = weakest_trade.entry_price - weakest_trade.stop_price
                        weakest_trade.pnl_dollars = (exit_price - weakest_trade.entry_price) * weakest_trade.shares - 2 * weakest_trade.shares * COMMISSION_PER_SHARE
                        weakest_trade.r_multiple = (exit_price - weakest_trade.entry_price) / risk_per_share_w if risk_per_share_w > 0 else 0
                        capital += exit_price * weakest_trade.shares - weakest_trade.shares * COMMISSION_PER_SHARE
                        closed_trades.append(weakest_trade)
                        del open_positions[weakest_sym]
                else:
                    continue  # not strong enough to rotate, skip
            if len(open_positions) >= MAX_POSITIONS:
                continue
