...
def load_bars() -> dict[str, pd.DataFrame]:
    ...
    for f in files:
        ...
        # Quality filter indicators (added vs vanilla Andrea)
        df["ma200"] = df["close"].rolling(200, min_periods=200).mean()
        df["ret_60d"] = df["close"].pct_change(60)  # used as RS proxy
...
def find_signals(
    ...
    for sym, df in bars.items():
        ...
        # 0) QUALITY FILTER (added vs vanilla Andrea):
        # Stock must be in long-term uptrend (above 200d MA) AND have positive 60d return
        ma200 = row.get("ma200")
        if pd.isna(ma200) or close_today <= float(ma200):
            continue
        ret_60d = row.get("ret_60d")
        if pd.isna(ret_60d) or ret_60d <= 0:
            continue
